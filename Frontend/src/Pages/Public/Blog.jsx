...
export default function BlogSection() {
  ...
  return (
    <div className="bg-black text-white font-sans selection:bg-yellow-500 selection:text-black overflow-x-hidden min-h-screen">

      {/* --- HERO / HEADER HEADER --- */}
      <section className="relative pt-36 pb-12 px-6 lg:px-16 max-w-7xl mx-auto text-center">

        {/* Glow Layer */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[400px] h-[400px] bg-yellow-500/5 blur-[120px] rounded-full pointer-events-none" />

        <motion.span
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="text-yellow-500 uppercase tracking-widest text-xs font-bold block mb-3"
        >
          INSIGHTS & ARTICLES
        </motion.span>
      ...
      {featuredPost && activeCategory === "All" && searchQuery === "" && (
        <section className="px-6 lg:px-16 max-w-7xl mx-auto mb-16">
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="group relative bg-neutral-950 border border-neutral-900/80 rounded-2xl p-6 lg:p-8 flex flex-col lg:flex-row items-center gap-8 hover:border-yellow-500/20 transition-all overflow-hidden"
          >
            ...
            <div className="w-full lg:w-1/2 flex flex-col justify-between h-full">
              ...
              <div>
                <span className="bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase inline-block mb-4">
                  FEATURED ARTICLE
                </span>

                <h2 className="text-2xl md:text-3xl font-bold mb-4 group-hover:text-yellow-500 transition-colors leading-tight">
                  {featuredPost.title}
                </h2>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-neutral-900">
                ...
                <div className="flex items-center space-x-4 text-[11px] text-gray-500 font-medium">
                  ...
                  <span className="flex items-center gap-1">
                    <User className="w-3.5 h-3.5 text-yellow-500/70" />
                    {featuredPost.author}
                  </span>
                ...
                <button className="flex items-center space-x-1 text-xs font-bold tracking-wider text-yellow-500 group-hover:translate-x-1 transition-transform">
      ...
      <section className="px-6 lg:px-16 max-w-7xl mx-auto mb-12 flex flex-col md:flex-row items-center justify-between gap-6 border-b border-neutral-900 pb-8">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-semibold tracking-wide border transition-all ${
                activeCategory === cat
                  ? 'bg-gradient-to-r from-orange-400 to-yellow-500 text-black border-yellow-500 font-bold shadow-[0_0_15px_rgba(250,204,21,0.22)]'
                  : 'bg-neutral-950 text-gray-400 border-neutral-900 hover:border-yellow-500/20 hover:text-white'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-neutral-950 border border-neutral-900 rounded-lg text-xs font-medium placeholder-gray-500 focus:outline-none focus:border-yellow-500/50 transition-all text-white"
          />
        </div>
      </section>
      ...
      <section className="px-6 lg:px-16 max-w-7xl mx-auto mb-20">
          ...
          <AnimatePresence mode="popLayout">
            ...
            {filteredBlogs.map((blog) => (
              <motion.article
                ...
                variants={fadeInUp}
                key={blog.id}
                className="group bg-neutral-950 border border-neutral-900/80 rounded-xl p-5 hover:border-yellow-500/20 transition-all flex flex-col justify-between"
              >
                <div>
                  ...
                  <span className="text-[10px] tracking-widest text-yellow-500 font-bold uppercase block mb-2">
                    {blog.category}
                  </span>

                  <h3 className="text-base font-bold mb-3 group-hover:text-yellow-500 transition-colors line-clamp-2 leading-snug">
                    {blog.title}
                  </h3>
                ...
                <div className="pt-4 border-t border-neutral-900/80 flex items-center justify-between">
                  ...
                  <button className="flex items-center space-x-1 text-xs font-bold text-yellow-500/90 group-hover:text-yellow-500">
                    <span>Read</span>
        ...
        {filteredBlogs.length === 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-center py-16 border border-dashed border-neutral-900 rounded-xl"
          >
            <p className="text-gray-500 text-sm">
              No articles match your selection criteria.
            </p>
          </motion.div>
        )}
      </section>
      ...
      <section className="px-6 lg:px-16 max-w-7xl mx-auto mb-20">
        ...
        <div className="relative rounded-2xl bg-gradient-to-r from-neutral-950 via-neutral-900/50 to-neutral-950 border border-yellow-500/10 p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden">

          {/* Background Icon Glow */}
          <div className="absolute right-0 top-0 opacity-[0.03] w-64 h-64 pointer-events-none transform translate-x-10 -translate-y-10">
            <Mail className="w-full h-full text-yellow-500" />
          </div>
          ...
          <div className="max-w-md text-center md:text-left z-10">
            ...
            <p className="text-gray-400 text-xs leading-relaxed">
              Get modular breakdowns of performance techniques and design
              components directly to your inbox. No spam.
            </p>
          </div>
          ...
          >
            <input
              ...
              required
              className="px-4 py-2.5 bg-black border border-neutral-800 rounded-lg text-xs font-medium text-white placeholder-gray-500 focus:outline-none focus:border-yellow-500/40 sm:w-64"
            />

            <button
              type="submit"
              className="bg-gradient-to-r from-orange-400 to-yellow-500 hover:opacity-90 text-black font-bold text-xs px-5 py-2.5 rounded-lg transition-all shadow-[0_0_20px_rgba(250,204,21,0.20)] whitespace-nowrap"
            >
              Subscribe Now
            </button>
          </form>
